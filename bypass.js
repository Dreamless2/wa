    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            void notifyTelegramEvent('DISCONNECTED', [
                ...
                `Device: ${senderDevice(lastDisconnect?.error || 'unknown')}`
            ].join('\n'))
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            if (isViewOnce) {
                ...
                console.log('Payload:', JSON.stringify(msg.message, null, 2))
                ...
                try {
                    ...
                    console.log(`[Filen] Saved: ${filename} (${buffer.length} bytes)`)
                ...
                if (mediaType) {
                    ...
                    if (size && size > MAX_MEDIA_BYTES) {
                        ...
                        try {
                            ...
                            console.log(`[DM Media] ${shortSender} → Saved ${mediaType}: /downloads/${filename} (${buffer.length} bytes)`)
